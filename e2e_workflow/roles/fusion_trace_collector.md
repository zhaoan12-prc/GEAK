...
`EXTRA_SERVER_ARGS`, `EXTRA_ENV`, `BACKEND`,
...
3. Otherwise run the existing `EVAL_DIR/bench_e2e.sh` serving capture with
   ...
   extra `profile/` or `round_` component around it. `GEAK_FUSION_TRACE=1`
   switches the capture out of the native profiler's statistical window into
   fusion evidence mode; what that means is per-backend and the adapter owns it:
   ...
   - **vllm** — `adapters/vllm.sh` turns `torch_profiler_with_stack` ON (its ordinary
     ...
     **Add `--compilation-config.cudagraph_mode=NONE` to the fusion capture's server args.**
     ...
     - **Skip the prefill ramp in engine steps, not seconds.** Set
       `GEAK_FUSION_DELAY_ITERS` (→ `ProfilerConfig.delay_iterations`) to at least
       `ceil(CONC*ISL/max_num_batched_tokens)`. Measured: at ISL 8192 / CONC 64 the ramp is
       ~32 steps, so a 25 s wall-clock warmup produced a 24-iteration window with ZERO decode
       steps, while a 180 s warmup outlived the background load and fell back to a cold-start
       capture — prefill again. Phase 1 reports the first case exactly —
       `decode_evidence: mixed_trace_no_decode_steps_in_window` — which means the annotation
       worked and the WINDOW was mis-placed; it is not `no_phase_annotation_in_trace`.
...
5. Require the completed capture to publish exactly `TRACE_MANIFEST_JSON`
   ...
   exits and the manifest has been validated. Its `phase_annotation_count` /
   `phase_annotation_dialects` counters are the objective check on step 2 — this
   is what decides `phase_evidence_status`, so read it rather than assuming:
