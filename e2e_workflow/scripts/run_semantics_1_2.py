...
import structural_pattern_mapping
...
def run(config_path, trace_path, shape_log_path, out_dir,
    ...
    patterns = structural_pattern_mapping.build(
        config_path, config_key, runtime_sources)
    ...
    status = "pass" if (
        semantic["status"] != "fail" and merged["status"] == "pass"
    ) else "fail"
    result = {
        "schema_version": 1,
